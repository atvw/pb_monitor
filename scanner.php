...
$last_scanned_records = @mysqli_query($scandb, "SELECT `scanned` FROM monitor_scanned WHERE `acct` = '$acct' ORDER BY `scanned` DESC LIMIT 1");
...
$baseline_results = @mysqli_query($scandb,"SELECT `file_path`, `file_hash`, `file_last_mod` FROM monitor_baseline WHERE `acct` = '$acct' ORDER BY `file_path` ASC");
	...
	if (!$iter->isDot() && !(in_array($iter->getSubPath(), $skip)))
	{
			...
			// OR extensionless AND extensionless is allowed
		{
			...
			if (!array_key_exists($file_path, $baseline))
			{
				...
				@mysqli_query($scandb, "INSERT INTO monitor_baseline SET `file_path` = '$file_path', `file_hash` = '" . $added[$file_path]['file_hash'] . "', `file_last_mod` = '" . $added[$file_path]['file_last_mod'] . "', `acct` = '$acct'");
				...
				if(!$firstscan) 
				{
					@mysqli_query($scandb, "INSERT INTO monitor_history SET `stamp` = '" . date('Y-m-d h:i:s') . "', `status` = 'Added', `file_path` = '$file_path', `hash_org` = 'Not Applicable', `hash_new` = '" . $added[$file_path]['file_hash'] . "', `file_last_mod` = '" . $added[$file_path]['file_last_mod'] . "', `acct` = '$acct'");
					if ($testing && mysqli_error($scandb)) echo mysqli_error($scandb);
				}  else {
					//	First Scan entry into history table
 					@mysqli_query($scandb, "INSERT INTO monitor_history SET `stamp` = '" . date('Y-m-d h:i:s') . "', `status` = 'Added', `file_path` = 'FIRST SCAN (file listings inhibited)', `hash_org` = 'Not Applicable', `hash_new` = 'Not Applicable', `file_last_mod` = 'Not Applicable', `acct` = '$acct'");
					if ($testing && mysqli_error($scandb)) echo mysqli_error($scandb);
				...
				if ($baseline[$file_path]['file_hash'] <> $current[$file_path]['file_hash'] || $baseline[$file_path]['file_last_mod'] <> $current[$file_path]['file_last_mod'])
				{
					...
					@mysqli_query($scandb,"UPDATE monitor_baseline SET `file_hash` = '" . $altered[$file_path]['hash_new'] . "', `file_last_mod` = '" . $altered[$file_path]['file_last_mod'] . "' WHERE `file_path` = '$file_path' AND `acct` = '$acct'");
					if ($testing && mysqli_error($scandb)) echo mysqli_error($scandb);

					//	INSERT altered file info in history table
					@mysqli_query($scandb,"INSERT INTO monitor_history SET `stamp` = '" . date('Y-m-d h:i:s') . "', `status` = 'Altered', `file_path` = '$file_path', `hash_org` = '" . $altered[$file_path]['hash_org'] . "', `hash_new` = '" . $altered[$file_path]['hash_new'] . "', `file_last_mod` = '" . $altered[$file_path]['file_last_mod'] . "', `acct` = '$acct'");
					if ($testing && mysqli_error($scandb)) echo mysqli_error($scandb);
				}
			}
...
foreach($deleted as $key => $value)
{
	...
	mysqli_query($scandb,"DELETE FROM monitor_baseline WHERE `file_path` = '$key' LIMIT 1");
	...
	@mysqli_query($scandb, "INSERT INTO monitor_history SET `stamp` = '" . date('Y-m-d h:i:s') . "', `status` = 'Deleted', `file_path` = '$key', `hash_org` = '" . $deleted[$key]['file_hash'] . "', `hash_new` = 'Not Applicable', `file_last_mod` = '" . $deleted[$key]['file_last_mod'] . "', `acct` = '$acct'");
	if ($testing && mysqli_error($scandb)) echo mysqli_error($scandb);
}
...
{  
	...
	@mysqli_query($scandb,"INSERT INTO monitor_history SET `stamp` = '" . date('Y-m-d h:i:s') . "', `status` = 'Unchanged', `file_path` = '$path', `hash_org` = 'Not Applicable', `hash_new` = 'Not Applicable', `file_last_mod` = 'Not Applicable', `acct` = '$acct'");
	if ($testing && mysqli_error($scandb)) echo mysqli_error($scandb);

	// update scanned table
	@mysqli_query($scandb,"INSERT INTO monitor_scanned SET `scanned` = '" . date('Y-m-d h:i:s') . "', `changes` = '$count_changes', `acct` = '$acct'");  
	if ($testing && mysqli_error($scandb)) echo mysqli_error($scandb);
...
@mysqli_query($scandb,"DELETE FROM monitor_history WHERE `stamp` < DATE_SUB(NOW(), INTERVAL 30 DAY)");
if ($testing && mysqli_error($scandb)) echo "History table clean-up problem: " . mysqli_error($scandb) . "<br />";

@mysqli_query($scandb,"DELETE FROM monitor_scanned WHERE `scanned` < DATE_SUB(NOW(), INTERVAL 30 DAY)");
